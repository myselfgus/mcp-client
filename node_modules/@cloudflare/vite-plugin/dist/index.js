...
const getFormData = (options) => {
	if (options.formData) {
		...
		const process$2 = async (key, value) => {
			if (isString$1(value)) formData.append(key, value);
...
const getHeaders = async (config, options) => {
	...
	else if (isString$1(options.body)) headers["Content-Type"] = "text/plain";
	else headers["Content-Type"] = "application/json";
	return new Headers(headers);
};
const getRequestBody = (options) => {
	if (options.body !== void 0) if (options.mediaType?.includes("/json")) return JSON.stringify(options.body);
	else if (isString$1(options.body) || isBlob(options.body)) return options.body;
...
const getResponseHeader = (response, responseHeader) => {
	if (responseHeader) {
		...
		if (isString$1(content)) return content;
	}
};
...
async function dockerLoginManagedRegistry(pathToDocker) {
	const credentials = await ImageRegistriesService.generateImageRegistryCredentials(getCloudflareContainerRegistry(), {
...
async function prepareContainerImagesForDev(args) {
	...
	for (const options of containerOptions) {
		if ("dockerfile" in options) {
			...
			if (!isCloudflareRegistryLink(options.image_uri)) throw new UserError(`Image "${options.image_uri}" is a registry link but does not point to the Cloudflare container registry.\nTo use an existing image from another repository, see https://developers.cloudflare.com/containers/platform-details/image-management/#using-pre-built-container-images`);
...
var require_ignore = /* @__PURE__ */ __commonJS({ "../../node_modules/.pnpm/ignore@5.3.1/node_modules/ignore/index.js": ((exports, module) => {
	...
	const checkPath = (path$3, originalPath, doThrow) => {
		if (!isString(path$3)) return doThrow(`path must be a string, but got \`${originalPath}\``, TypeError);
...
var PluginContext = class {
	...
	get resolvedPluginConfig() {
		if (!this.#resolvedPluginConfig) throw new Error("Expected resolvedPluginConfig to be defined");
		return this.#resolvedPluginConfig;
	}
	get resolvedViteConfig() {
		if (!this.#resolvedViteConfig) throw new Error("Expected resolvedViteConfig to be defined");
...
function getEntryWorkerConfig(resolvedPluginConfig) {
	if (resolvedPluginConfig.type === "assets-only") return;
...
async function getDevMiniflareOptions(config) {
			...
			serviceBindings: {
				__VITE_HTML_EXISTS__: async (request$1) => {
					...
					if (pathname.endsWith(".html")) {
						...
						const publicDirInRoot = publicDir.startsWith(withTrailingSlash$1(root));
						const publicPath = withTrailingSlash$1(publicDir.slice(root.length));
	...
	let allContainerOptions = /* @__PURE__ */ new Map();
	const workersFromConfig = resolvedPluginConfig.type === "workers" ? await Promise.all(Object.entries(resolvedPluginConfig.workers).map(async ([environmentName, workerConfig]) => {
		...
		if (workerConfig.containers?.length && workerConfig.dev.enable_containers) {
			...
			for (const option of options ?? []) allContainerOptions.set(option.image_tag, option);
		}
...
pp$8.parseClassElement = function(constructorAllowsSuper) {
	...
	if (!keyName && ecmaVersion$1 >= 8 && this.eatContextual("async")) if ((this.isClassElementNameStart() || this.type === types$1.star) && !this.canInsertSemicolon()) isAsync$1 = true;
	else keyName = "async";
	if (!keyName && (ecmaVersion$1 >= 9 || !isAsync$1) && this.eat(types$1.star)) isGenerator = true;
	...
	if (ecmaVersion$1 < 13 || this.type === types$1.parenL || kind !== "method" || isGenerator || isAsync$1) {
...
pp$8.parseExportDefaultDeclaration = function() {
	var isAsync$1;
	if (this.type === types$1._function || (isAsync$1 = this.isAsyncFunction())) {
...
pp$5.parsePropertyValue = function(prop, isPattern, isGenerator, isAsync$1, startPos, startLoc, refDestructuringErrors, containsEsc) {
	if ((isGenerator || isAsync$1) && this.type === types$1.colon) this.unexpected();
...
function _resolve(id, options = {}) {
	...
	if (isAbsolute(id)) try {
		if (statSync$1(id).isFile()) return pathToFileURL$1(id);
	} catch (error) {
...
function assertIsNotPreview(resolvedPluginConfig) {
	assert(resolvedPluginConfig.type !== "preview", `Expected "assets-only" or "workers" plugin config`);
}
function assertIsPreview(resolvedPluginConfig) {
	assert(resolvedPluginConfig.type === "preview", `Expected "preview" plugin config`);
}
	...
	return [
		{
			...
			config(userConfig, env) {
				...
				if (!workersConfigsWarningShown) {
					...
					const workersConfigsWarning = getWarningForWorkersConfigs(ctx.resolvedPluginConfig.rawConfigs);
					if (workersConfigsWarning) console.warn(workersConfigsWarning);
				}
				return {
					appType: "custom",
					server: { fs: { deny: [
			...
			async configureServer(viteDevServer) {
				...
				assertIsNotPreview(ctx.resolvedPluginConfig);
				const inputInspectorPort = await getInputInspectorPortOption(ctx.resolvedPluginConfig, viteDevServer, miniflare);
				...
				const { config: miniflareDevOptions, allContainerOptions } = await getDevMiniflareOptions({
				...
				if (ctx.resolvedPluginConfig.type === "workers") {
					...
					if (allContainerOptions.size > 0) {
						...
						process.on("exit", async () => {
							if (allContainerOptions.size > 0) cleanupContainers(getDockerPath(), containerImageTagsSeen);
						});
					}
				}
				return () => {
			...
			async configurePreviewServer(vitePreviewServer) {
				...
				const inputInspectorPort = await getInputInspectorPortOption(ctx.resolvedPluginConfig, vitePreviewServer);
				const { config: miniflareOptions, allContainerOptions } = await getPreviewMiniflareOptions({
					resolvedPluginConfig: ctx.resolvedPluginConfig,
					vitePreviewServer,
					inspectorPort: inputInspectorPort
				});
		...
		},
		{
			...
			configureServer(viteDevServer) {
				...
				if (ctx.resolvedPluginConfig.type === "workers" && pluginConfig.inspectorPort !== false) addDebugToVitePrintUrls(viteDevServer);
				...
				viteDevServer.middlewares.use(DEBUG_PATH, async (_, res, next) => {
					const resolvedInspectorPort = await getResolvedInspectorPort(ctx.resolvedPluginConfig, miniflare);
		...
		},
		{
			...
			async configureServer(viteDevServer) {
				...
				if (ctx.resolvedPluginConfig.type === "workers") {
					...
					assert(entryWorkerConfig, `No entry Worker config`);
					const entryWorkerName = entryWorkerConfig.name;
					viteDevServer.middlewares.use("/cdn-cgi/handler/", (req, res, next) => {
						createRequestHandler((request$1) => {
							assert(miniflare, `Miniflare not defined`);
